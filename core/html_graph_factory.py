...
# ---------------------------------------------------------------------------
# Bivariate legend HTML
# ---------------------------------------------------------------------------

def build_bivariate_legend_html(
    x_label: str,
    y_label: str,
    matrix: List[List[str]],  # hex colour strings
) -> str:
    """
    Build an HTML bivariate legend matrix.

    Parameters
    ----------
    x_label, y_label : str
        Axis labels (e.g. "Exposure →", "Sensitivity →").
    matrix : list[list[str]]
        N×N grid of hex colour codes.
    ...
    for row_idx, row in enumerate(matrix):
        for col_idx, colour in enumerate(row):
            cells.append(
                f'<rect x="{col_idx * cell_size}" y="{row_idx * cell_size}" '
                f'width="{cell_size}" height="{cell_size}" fill="{colour}" />'
            )

    svg = f"""<svg width="{total + 40}" height="{total + 40}"
     xmlns="http://www.w3.org/2000/svg">
  <g transform="translate(30, 10)">
    {"".join(cells)}
    <text x="{total / 2}" y="{total + 20}" text-anchor="middle"
     font-family="Inter,sans-serif" font-size="11" fill="#333">{x_label}</text>
    <text x="-12" y="{total / 2}" text-anchor="middle"
     font-family="Inter,sans-serif" font-size="11" fill="#333"
     transform="rotate(-90, -12, {total / 2})">{y_label}</text>
  </g>
</svg>"""
...
<div class="planx-card" style="text-align:center;">
...
def build_micro_bar_chart_html(
    title: str,
    categories: List[str],
    values: List[float],
    bar_colour: str = "#e94560",
) -> str:
    """Build an HTML micro bar chart (pure CSS bars, no JS)."""
    ...
    for cat, val in zip(categories, values):
        ...
        bars_html += (
            ...
            f'<span style="display:inline-block;width:80px;font-size:11px;">{cat}</span>'
            f'<span style="display:inline-block;width:{pct}%;height:14px;'
            f'background:{bar_colour};border-radius:3px;min-width:4px;"></span>'
            f'<span style="font-size:11px;margin-left:4px;">{val:.1f}</span>'
            f'</div>\n'
        )
